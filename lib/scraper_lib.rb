...
class Scraper
  ...
  def get_post_url(url, css_class)
    ...
    @url_array.collect do |string|
      string.prepend('https://dev.to')
    end
    @url_array.delete_if(&:empty?)
    @url_array
  end
  ...
  def hashify_els
    ...
    author_array.zip(title_array, post_urls) { |author, title, post| list[author] = title, post }
    pp list
  end
end
